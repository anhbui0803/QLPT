...
class AuthLogic:
    ...
    async def save_user(self, account_data: CreateAccountSchema):
        if await self.__repository.get_account_by_email(
    ...
    async def _authenticate(self, email: str, password: str):
        if not (account := await self.__repository
                .get_account_by_email(email=email)) \
    ...
    async def login(self, email: str, password: str):
        ...
        return {'token': await self.__token_service.encode_token(
            email=account['email'])}

    async def get_user_by_email(self, email: str):
        user = await self.__repository.get_account_by_email(email=email)
        if not user:
